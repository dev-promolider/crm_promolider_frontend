        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="form-section">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                <label>Video/Imagen Promocional</label>
                <label for="promoFile" class="custom-file-upload" :class="{ 'has-file': promoFile || promoPreviewUrl }">
                  ...
                  <div class="upload-text">
                    <span v-if="!promoFile && !promoPreviewUrl"><strong>Haz clic para subir</strong> o arrastra un archivo</span>
      ...
      <div class="vcr-preview">
        ...
        <div class="vcr-banner">
          <div class="vcr-banner-content">
            <h1 class="vcr-title">{{ form.title || 'Título del Libro' }}</h1>
            <p class="vcr-subtitle">{{ form.description || 'Descripción breve...' }}</p>
            <div class="vcr-meta">
              <span class="vcr-bestseller">EBOOK</span>
              <span class="vcr-rating">5.0 ★★★★★ <small>(0 valoraciones)</small></span>
              <span class="vcr-students">0 lectores</span>
            </div>
            <div class="vcr-meta-bottom">
              <p>Creado por <strong>Autor</strong></p>
              <p>Última actualización: Reciente</p>
            </div>
          </div>
        </div>
        ...
        <div class="vcr-body-wrapper">
          <div class="vcr-body">
            ...
            <div class="vcr-section">
              <h2>Este libro incluye:</h2>
              <ul class="vcr-includes">
                <li><BookOpen size="18" /> Libro digital completo</li>
                <li><Download size="18" /> Descarga inmediata</li>
                <li><Smartphone size="18" /> Acceso en dispositivos móviles</li>
                <li><InfinityIcon size="18" /> Acceso de por vida</li>
              </ul>
            </div>
            ...
            <div class="vcr-section vcr-description-section" v-if="form.description || form.course_about">
              ...
              <h3 v-if="form.course_about" style="font-size: 15px; margin-bottom: 8px;">Acerca de este libro:</h3>
            ...
            <div class="vcr-buy-card">
              <div class="vcr-buy-img">
                ...
                <template v-else>
                  ...
                  <div v-if="promoPreviewUrl && isPromoVideo" class="play-overlay" @click="isPlayingPromo = true">
                    <PlayCircle class="play-icon" />
                    <span class="play-text">Vista previa de este libro</span>
                  </div>
                </template>
              </div>
              <div class="vcr-buy-body">
                <h2 class="vcr-price">
                  <span v-if="salePrice > 0">${{ salePrice }}</span>
                  <span v-else class="text-success">GRATIS</span>
                  <small v-if="hasDiscount" class="old-price">${{ basePrice }}</small>
                </h2>
                <button class="vcr-btn-primary">Obtener libro ahora</button>
                <ul class="vcr-buy-perks">
                  <li><Zap size="14" /> <strong>Acceso inmediato</strong></li>
                  <li><Download size="14" /> <strong>Descarga en PDF</strong></li>
...
import { BookOpen, Download, Smartphone, Infinity as InfinityIcon, Lock, Zap, CheckCircle2, PlayCircle, ImageIcon, Film } from 'lucide-vue-next';
...
const isPromoVideo = ref(false);
const isPlayingPromo = ref(false);
const uploadProgress = ref(0);

const basePrice = computed(() => parseFloat(form.value.price_base) || 0);
const salePrice = computed(() => parseFloat(form.value.price) || 0);
const hasDiscount = computed(() => basePrice.value > salePrice.value && salePrice.value > 0);
...
const handlePromoUpload = (event) => {
  if (event.target.files.length > 0) {
    ...
    isPromoVideo.value = promoFile.value.type.startsWith('video/');
