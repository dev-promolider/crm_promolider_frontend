<template>
  <div class="pickle-layout h-full min-h-[calc(100vh-100px)] w-full flex flex-col md:flex-row gap-6 p-4 lg:p-6 bg-[#09090b] text-slate-100 relative overflow-hidden rounded-xl">
    ...
    <div class="sidebar w-full md:w-72 glass-panel flex flex-col p-4 md:p-6 rounded-2xl z-10 shrink-0">
      <button @click="startNewChat" class="new-chat-btn w-full bg-primary-500 hover:bg-primary-600 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-200 mb-5 flex items-center justify-center gap-2 shadow-lg shadow-primary-500/20">
      ...
      <div class="chat-list flex-1 overflow-y-auto pr-2 flex flex-col gap-2">
        ...
        <div v-else v-for="chat in chats" :key="chat.id" 
             ...
             :class="currentChatId === chat.id ? 'bg-primary-500/15 border-primary-500/30 text-white' : 'bg-white/5 hover:bg-white/10 text-slate-300'"
    ...
    <div class="chat-container flex-1 glass-panel flex flex-col rounded-2xl z-10 overflow-hidden shadow-2xl">
      ...
      <div class="chat-header p-5 border-b border-white/10 flex items-center justify-between bg-black/20">
        ...
        <div class="relative hidden sm:block">
          <select v-model="selectedModel" @change="switchModel" class="appearance-none bg-white/5 hover:bg-white/10 border border-white/10 text-sm text-slate-200 py-2 pl-3 pr-8 rounded-xl outline-none focus:border-primary-500/50 focus:ring-2 focus:ring-primary-500/20 transition-all cursor-pointer">
        ...
        <template v-for="(msg, index) in messages" :key="index">
          ...
          <div v-if="msg.type === 'form'" class="message assistant self-start bg-white/5 border border-white/10 rounded-2xl rounded-bl-sm py-4 px-5 max-w-[85%] animate-fade-in-up w-full md:w-auto">
            ...
            <div v-for="(q, qIndex) in msg.content.questions" :key="q.id" class="mb-5">
              <div class="font-semibold text-sm mb-3 text-primary-100">{{ qIndex + 1 }}. {{ q.question }}</div>
              ...
              <div class="flex flex-wrap gap-2">
                <button v-for="ans in q.suggestedAnswers" :key="ans" 
                        ...
                        :class="isFormOptionSelected(msg, q.id, ans) ? 'bg-primary-500 border-primary-500 text-white shadow-md shadow-primary-500/30' : 'bg-white/5 border-white/10 hover:bg-white/10 text-slate-300'">
                  {{ ans }}
                </button>
              </div>
            ...
            <button @click="submitForm(msg)" 
                    ...
                    class="w-full mt-2 bg-primary-500 hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed text-white py-2.5 rounded-xl font-medium transition-all text-sm flex items-center justify-center gap-2">
...
const userId = computed(() => {
  return Number(authStore.user?.id) || 1; // Fallback to 1 if no user (should not happen in protected route)
});
