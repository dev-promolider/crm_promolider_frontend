<template>
  <div class="container">
    ...
    <div class="card shadow-lg">
            ...
            <div v-for="(image, index) in item.images" :key="index" class="carousel-item" :class="{ active: index === currentImageIndex }">
              <div class="image-container">
                ...
                <div class="text-overlay">
                  ...
                  <span><Clock :size="14" /> {{ item.duration || 0 }} horas</span>
                  <span><Signal :size="14" /> {{ getLevelText(item.level) }}</span>
      ...
      <div v-else>
        <div class="image-container">
          <img :src="item.images && item.images[0] ? item.images[0].image : ''" class="img-fluid w-100" alt="Mini Curso" />
          <div v-if="!item.images || !item.images[0]" class="img-placeholder"><Film :size="48" style="color:#ccc" /></div>
          <div class="text-overlay">
            <h2>{{ item.title }}</h2>
            <span>{{ item.category_name }}</span>
            <span><Clock :size="14" /> {{ item.duration || 0 }} horas</span>
      ...
      <div class="info-section">
        <div class="info-main">
          ...
          <div class="info-block" v-if="item.modules && item.modules.length > 0">
            <h4><Film :size="18" /> Módulos</h4>
            <div v-for="(mod, index) in item.modules" :key="mod.id" class="module-item">
              <h6>Módulo {{ index + 1 }}: {{ mod.title }}</h6>
              <p class="module-content">{{ mod.content }}</p>
              <small>{{ mod.duration }} horas</small>
              <hr v-if="index < item.modules.length - 1" />
            </div>
          </div>
          <div class="info-block" v-if="item.documents && item.documents.length > 0">
            <h4><FileDown :size="18" /> Documentos</h4>
            <ul class="doc-list">
              <li v-for="doc in item.documents" :key="doc.id"><a :href="doc.document" target="_blank">Descargar Documento</a></li>
            </ul>
          </div>
        </div>
        <div class="info-sidebar">
          <div class="info-block">
            <h4><Film :size="18" /> Información del Curso</h4>
            <p><strong>Duración:</strong> {{ item.duration || 0 }} horas</p>
            <p><strong>Nivel:</strong> {{ getLevelText(item.level) }}</p>
            <p><strong>Categoría:</strong> {{ item.category_name || 'Sin categoría' }}</p>
            <p><strong>Módulos:</strong> {{ item.modules ? item.modules.length : 0 }}</p>
          </div>
          <div class="info-block">
    ...
    <div v-if="showModal" class="modal-backdrop" @click.self="closeModal">
      <div class="modal-content-card">
        <div class="modal-header"><h5><Share2 :size="18" /> Invitar a Mini Curso: {{ item.title }}</h5><button class="btn-close" @click="closeModal">✕</button></div>
        <div class="modal-body">
          <template v-if="!newInvitationLink">
            <div v-if="invitationData.existInvitation" class="alert alert-info">
              ...
              <p>Ya tienes un link de invitación activo.</p>
...
import {
  ChevronLeft, ChevronRight, Clock, Signal, Film, Info, FileDown,
  ShoppingCart, Download, Share2, MessageCircle, Facebook, Instagram, Clipboard
} from 'lucide-vue-next'

const route = useRoute()
const router = useRouter()
const item = ref({ images: [], modules: [], documents: [] })
...
function getWhatsappUrl() { return `https://wa.me/?text=${encodeURIComponent(`Te recomiendo este mini curso: ${item.value.title}`)}` }
function getFacebookUrl() { return `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(getCurrentUrl())}` }
function getWhatsappShareUrl() { return `https://wa.me/?text=${encodeURIComponent(`¡Hola! Te invito a conocer este mini curso: "${item.value.title}" - ${item.value.duration} horas. Regístrate usando mi link: ${getCurrentInvitationLink()}`)}` }
...
async function purchaseItem() {
  ...
  loading.value = true
  try {
    const res = await apiClient.post(`/marketing/mini-course/invitation/purchase/${item.value.id}`)
    if (res.data.success) {
      isPurchased.value = true
      showToastMessage(res.data.message || 'Mini curso registrado exitosamente')
...
async function checkPurchase() {
  if (!item.value.id) return
  try {
    const res = await apiClient.get(`/marketing/mini-course/invitation/check-purchase/${item.value.id}`)
    if (res.data.success) {
...
async function checkExistingInvitation() {
  if (!item.value.id) return
  try {
    const res = await apiClient.get(`/marketing/mini-course/invitation/check-invitation/${item.value.id}`)
    if (res.data.success) {
...
async function createInvitationLink() {
  ...
  try {
    const res = await apiClient.post(`/marketing/mini-course/invitation/invitation-link/${item.value.id}`)
...
onMounted(async () => {
  try {
    ...
    await checkPurchase()
  } catch (e) { console.error('Error loading mini course:', e) }
})
</script>
...
.buttons-container .btn, .purchased-actions .btn { width: fit-content; min-width: 160px; padding: 10px 24px; border: none; border-radius: 8px; font-weight: 600; font-size: 1rem; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; }
...
.module-item h6 { color: #007bff; margin-bottom: 8px; }
.module-content { color: #666; font-size: 14px; line-height: 1.5; }
