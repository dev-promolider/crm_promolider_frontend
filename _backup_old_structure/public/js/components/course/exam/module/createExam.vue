                ...
                <tbody>
                  <tr v-if="isEmpty">
                    <td colspan="7" class="text-center">
                      Ningún dato disponible en esta tabla, seleccione un módulo
                    </td>
                  </tr>
                  <tr
                    v-else
                    v-for="(exam, index) in this.exams"
                    v-bind:key="exam.id"
                    v-bind:index="index"
                  >
                    <td>
...
export default {
  ...
  methods: {
    ...
    listExams(module_id) {
      axios
        .get(`/course/exam/module/${module_id}/list`)
        .then((response) => {
    ...
    async submit() {
      if (this.validateFields()) {
        ...
        try {
          ...
          this.loading = false;
          this.listExams(this.mutableModule);
          this.initForm();
        } catch (error) {
          this.isSubmitting = false;
          this.loading = false;
    ...
    getOptionSelected(exam) {
      ...
      switch (option) {
        case '1':
          document.location.href = `/course/exam/module/edit/${exam.id}`;
          break;
        case '2':
          if (exam.status == 1) {
