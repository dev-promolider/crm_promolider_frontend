...
export default {
  ...
  methods: {
    ...
    listGames(module_id) {
      axios
        .get(`/course/game/module/${module_id}/list`)
        .then((response) => {
    ...
    async submit() {
      if (this.validateFields()) {
        ...
        try {
          ...
          const response =
            request.status == 200
              ? this.$message.success('La dinámica ha sido registrada correctamente')
              : await this.$message.warning('Error al validar datos');
          this.isSubmitting = false;
          this.loading = false;
          this.listGames(this.mutableModule);
          this.initForm();
        } catch (error) {
          this.isSubmitting = false;
          this.loading = false;
    ...
    async activateGame(id, course_id, lesson_id, module_id, game_type_id) {
      const formdata = new FormData();
      formdata.append('id', id); // game id
      formdata.append('course_id', course_id); // course id
      formdata.append('lesson_id', lesson_id); // lesson id
      formdata.append('module_id', module_id); // module id
      formdata.append('game_type_id', game_type_id); // game_type_id
      try {
        const request = await axios({
