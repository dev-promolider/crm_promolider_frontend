              ...
              <div class="row">
                <div class="col-12">
                  <div class="form-group">
                    <label class="d-block">Seleccione la condición que se tiene que cumplir para entregar un certificado</label>
                    <div class="d-inline py-2">
                      <div>
                        <input type="radio" v-model="conditionSelected" value="0" @click="showVideos = true" name="showVideo" id="radio1">
                        <label for="radio1">Visualizar videos</label>
                      </div>
                      <div>
                        <input type="radio" v-model="conditionSelected" value="1" @click="showVideos = false" name="showVideo" id="radio2">
                        <label for="radio2">Aprobar exámenes</label>
                      </div>
                      <div>
                        <input type="radio" v-model="conditionSelected" value="2" @click="showVideos = true" name="showVideo" id="radio3">
                  ...
                  <div class="form-group" v-if="showVideos">
                    ...
                    <el-select
                      v-model="form.type"
                      placeholder="Seleccione cuando entregar el certificado"
                      class="d-inline"
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <select v-model="form.customized_certificate" class="form-control">
                          ...
                          <option v-for="option in options" :value=option.label>{{option.value}}</option>
                        </select>
                    </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    validateFields() {
      ...
      if(this.form.customized_certificate == null || this.form.customized_certificate == undefined){
      ...
      if (this.validateFields()) {
        ...
        try {
          ...
          const request = await axios({
            url: `/course/certificate/store/configuration`,
            method: 'post',
            data: formdata,
          });
          const response =
    ...
    async getConfiguration() {
      try {
        this.loading = true;
        const request = await axios({
          url: `/course/certificate/configuration/` + this.course.id,
          method: 'get',
        });
